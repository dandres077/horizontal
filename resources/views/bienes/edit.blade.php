...
@section('title', 'Bienes'.' | '.config('app.name'))
        ...
        <div class="col-lg-12">
            ...
            <div class="kt-portlet kt-portlet--last kt-portlet--head-lg kt-portlet--responsive-mobile" id="kt_page_portlet">
                ...
                <div class="kt-portlet__body">
                    <form method="post" class="form-horizontal" action="{{ url('/bienes/'.$data->id.'/edit')}}" autocomplete="off" enctype="multipart/form-data">
                    {{ csrf_field()}}
                        <div class="row">
                                ...
                                <div class="kt-section kt-section--first">
                                    <div class="kt-section__body">
                                        ...
                                        <div class="form-group row">
                                            ...
                                            <div class="col-9">
                                                <select class="form-control" name="torre_id" id="torre_id">
                                                @foreach ($torres as $torre)
                                                    <option value="{{$torre->id}}" @if($data->torre_id==$torre->id) selected @endif> {{ $torre->nombre, $data->torre_id }}</option>
                                                @endforeach                                                
                                                </select>
                                            </div>
                                        </div>
                                        ...
                                        <div class="form-group row">
                                            <label class="col-3 col-form-label">Tipo inmueble</label>
                                            <div class="col-9">
                                                <select class="form-control" name="metrosbienes_id" id="metrosbienes_id">
                                                @foreach ($metros as $metro)
                                                    <option value="{{$metro->id}}" @if($data->metrosbienes_id==$metro->id) selected @endif> {{ $metro->nombre, $data->metrosbienes_id }}</option>
                                                @endforeach                                                
                                                </select>
                                            </div>
                                        </div>  
...
<script type="text/javascript">
  $('#conjunto_id').on('change', function(e){
    ...
    $.get('http://localhost/horizontal/public/api/torre/' + province_id,function(data) {
      console.log(data);

      $('#torre_id').empty();
      $('#metrosbienes_id').empty();
...
<script type="text/javascript">
  $('#conjunto_id').on('change', function(e){
    ...
    $.get('http://localhost/horizontal/public/api/tipoinmueble/' + province_id,function(data) {
      console.log(data);
      ...
      $.each(data, function(index, regenciesObj){
        $('#metrosbienes_id').append('<option value="'+ regenciesObj.id +'">'+ regenciesObj.nombre +'</option>');
      })
    });
  });
